...
interface PersonalizeButtonProps {
  chapterId: string;
  content: string;
  onPersonalizedContent: (personalizedContent: string) => void;
}
...
const PersonalizeButton: React.FC<PersonalizeButtonProps> = ({
  ...
  const handlePersonalize = async () => {
    ...
    try {
      const response = await fetch('/api/personalization/process', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="mt-4">
      <button
        ...
        disabled={loading}
        className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md disabled:opacity-50 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
      >
        {loading ? (
