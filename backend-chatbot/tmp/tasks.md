...
- [X] T003 Install and configure required dependencies (FastAPI, Cohere, Qdrant Client, SQLAlchemy, PyPDF2, etc.)
...
- [X] T006 [P] Create configuration module to load environment variables and initialize clients (config.py)
- [X] T007 [P] Implement database connection setup with Neon Postgres (using SQLAlchemy)
- [X] T008 [P] Create database models for chunk metadata based on data model (models/chunk_metadata.py)
- [X] T009 [P] Create Qdrant client wrapper for vector operations (services/vector_store.py)
- [X] T010 Implement text splitting utilities with overlap (utils/text_splitter.py)
...
- [X] T011 [US4] Create ingestion service to handle PDF, text, and Markdown files (services/ingestion.py)
- [X] T012 [US4] Implement PDF text extraction using PyPDF2/pdfplumber
- [X] T013 [US4] Implement text and Markdown processing
- [X] T014 [US4] Implement intelligent chunking with 500-1000 token size and overlap
- [X] T015 [US4] Generate embeddings using Cohere embed-english-v3.0
- [X] T016 [US4] Store vectors in Qdrant collection "book_collection" with payload
- [X] T017 [US4] Store chunk metadata in Neon Postgres with embedding_id references
- [X] T018 [US4] Create POST /ingest endpoint in main API (api/main.py)
- [X] T019 [US4] Implement error handling for malformed or corrupted content
- [X] T020 [US4] Add file format validation and response formatting
...
- [X] T021 [US1] Create RAG service with vector search functionality (services/rag.py)
- [X] T022 [US1] Implement top-k retrieval from Qdrant based on query embeddings
- [X] T023 [US1] Implement Cohere chat generation with retrieved documents
- [X] T024 [US1] Create POST /chat endpoint without selected text handling (api/main.py)
- [X] T025 [US1] Implement query embedding using Cohere API
- [X] T026 [US1] Add response formatting with source citations
- [X] T027 [US1] Implement error handling for ambiguous queries
...
- [X] T028 [US2] Update RAG service to handle selected text context (services/rag.py)
- [X] T029 [US2] Implement direct context injection when selected_text is provided
- [X] T030 [US2] Modify /chat endpoint to accept optional selected_text parameter
- [X] T031 [US2] Implement selected text truncation to 4000 token limit
- [X] T032 [US2] Add validation to ensure answers come only from selected text
...
- [X] T033 [US3] Create embeddable HTML widget with basic styling (frontend/widget.html)
- [X] T034 [US3] Implement JavaScript to capture text selection via window.getSelection()
- [X] T035 [US3] Implement API communication to /chat endpoint
- [X] T036 [US3] Create simple message history UI in the widget
- [X] T037 [US3] Add proper error handling and loading states
- [X] T038 [US3] Add styling to match common book interfaces
...
- [X] T039 [US1] [US2] Implement GET /health endpoint (api/main.py)

## Phase 8: Polish & Cross-Cutting Concerns

- [X] T040 Add comprehensive error handling throughout the application
- [X] T041 Implement logging for debugging and monitoring
...
- [X] T044 Add unit tests for core functionality (services/ingestion.py, services/rag.py)
- [ ] T045 Perform end-to-end testing of complete workflow
- [ ] T046 Optimize performance for response time under 10 seconds
- [X] T047 Document API endpoints with examples
- [X] T048 Create .env.example with all required environment variables
- [X] T049 Add input validation for all API endpoints